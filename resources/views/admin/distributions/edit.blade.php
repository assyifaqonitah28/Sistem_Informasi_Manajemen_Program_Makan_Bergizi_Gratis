...
@section('page-title', 'Edit Distribusi')
@section('breadcrumb')
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm p-6 md:p-8">
        <h2 class="text-xl font-bold text-gray-800 mb-6 flex items-center">
            <i class="fas fa-edit mr-2 text-blue-600"></i> Edit Distribusi
        </h2>

        <form method="POST" action="{{ route('admin.distributions.update', $distribution) }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <select name="program_id" class="form-select @error('program_id') border-red-500 @enderror">
                        ...
                        @foreach($programs as $id => $name)
                            <option value="{{ $id }}" {{ old('program_id', $distribution->program_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <select name="beneficiary_id" class="form-select @error('beneficiary_id') border-red-500 @enderror">
                        ...
                        @foreach($beneficiaries as $b)
                            <option value="{{ $b->id }}" {{ old('beneficiary_id', $distribution->beneficiary_id) == $b->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="date" name="distribution_date" value="{{ old('distribution_date', $distribution->distribution_date->format('Y-m-d')) }}" class="form-input @error('distribution_date') border-red-500 @enderror">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="quantity" value="{{ old('quantity', $distribution->quantity) }}" min="1" class="form-input @error('quantity') border-red-500 @enderror">
                ...
                <div class="space-y-2">
                    ...
                    <select name="status" class="form-select @error('status') border-red-500 @enderror">
                        <option value="scheduled" {{ old('status', $distribution->status) == 'scheduled' ? 'selected' : '' }}>Terjadwal</option>
                        <option value="distributed" {{ old('status', $distribution->status) == 'distributed' ? 'selected' : '' }}>Terdistribusi</option>
                        <option value="failed" {{ old('status', $distribution->status) == 'failed' ? 'selected' : '' }}>Gagal</option>
                        <option value="cancelled" {{ old('status', $distribution->status) == 'cancelled' ? 'selected' : '' }}>Dibatalkan</option>
                    </select>
            ...
            <div class="space-y-2">
                ...
                <textarea name="notes" rows="3" class="form-textarea @error('notes') border-red-500 @enderror">{{ old('notes', $distribution->notes) }}</textarea>
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-gray-200">
                ...
                <button type="submit" class="btn btn-primary w-full sm:w-auto"><i class="fas fa-save mr-2"></i> Update Distribusi</button>
            </div>
        </form>
    </div>
</div>
